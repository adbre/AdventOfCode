...
namespace AdventOfCode
{
    ...
    {
        [Test]
        [TestCase("turn on 0,0 through 999,999", ExpectedResult = 1000*1000)]
        [TestCase("toggle 0,0 through 999,0", ExpectedResult = 1000)]
        [TestCase("turn on 0,0 through 999,999\r\nturn off 499,499 through 500,500", ExpectedResult = 1000 * 1000 - 4)]
        [TestCase(PuzzleInput, ExpectedResult = 569999, TestName = "Answer Part1")]
        public int Part1(string input)
        {
            var grid = new LightGrid();
            grid.ExecuteInstructions(input);
            return grid.LitLamps;
        }
        [Test]
        [TestCase("turn on 0,0 through 0,0", ExpectedResult = 1)]
        [TestCase("toggle 0,0 through 999,999", ExpectedResult = 2000000)]
        [TestCase("toggle 0,0 through 999,999\r\nturn off 499,499 through 500,500", ExpectedResult = 2000000 - 4)]
        [TestCase(PuzzleInput, ExpectedResult = 17836115, TestName = "Answer Part2")]
        ...
        {
            private static readonly Regex InstructionRegex = new Regex(@"^(turn (?:on|off)|toggle) ([0-9]+),([0-9]+) through ([0-9]+),([0-9]+)$");

            private readonly Lamp[,] _lamps = new Lamp[1000,1000];

            public LightGrid()
            {
                for (var x = 0; x < _lamps.GetLength(0); x++)
                {
                    for (var y = 0; y < _lamps.GetLength(1); y++)
                    {
                        _lamps[x, y] = CreateNewLamp();
                    }
                }
            }

            private Lamp CreateNewLamp()
            {
                var lamp = new Lamp();
                lamp.PropertyChanged += (sender, args) =>
                {
                    if (args.PropertyName == nameof(Lamp.Lit))
                        LitLamps += ((Lamp)sender).Lit ? 1 : -1;
                };

                lamp.BrightnessChanged += (sender, args) =>
                {
                    TotalBrightness += args.NewBrightness - args.OldBrightness;
                };
            ...
            public void ExecuteInstructions(string lines)
            {
                foreach (var line in lines.ReadLines(true))
            ...
            public void ExecuteInstruction(string line)
            {
                ...
                var x1 = int.Parse(match.Groups[2].Value, formatProvider);
                var y1 = int.Parse(match.Groups[3].Value, formatProvider);
                var x2 = int.Parse(match.Groups[4].Value, formatProvider);
                var y2 = int.Parse(match.Groups[5].Value, formatProvider);

                var op = ToOperation(operation);

                foreach (var lamp in Range(x1, y1, x2, y2))
                    op(lamp);
            }

            private Action<Lamp> ToOperation(string operation)
            {
                switch (operation)
                {
                    case "toggle": return lamp => lamp.Toggle();
                    case "turn on": return lamp => lamp.TurnOn();
                    case "turn off": return lamp => lamp.TurnOff();
                }

                throw new ArgumentOutOfRangeException();
            }

            private IEnumerable<Lamp> Range(int x1, int y1, int x2, int y2)
            {
                for (var x = x1; x <= x2; x++)
                {
                    for (var y = y1; y <= y2; y++)
                    {
                        yield return _lamps[x, y];
                    }
                }
            }

            private class Lamp : INotifyPropertyChanged
            {
                private bool _lit;
                ...
                public int Brightness
                {
                    ...
                    set
                    {
                        ...
                        OnBrightnessChanged(oldBrightness);
                    }
                }
                ...
                protected virtual void OnBrightnessChanged(int oldBrightness)
                {
                    BrightnessChanged?.Invoke(this, new BrightnessChangedEventArgs(oldBrightness, Brightness));
                }
            }

            private delegate void BrightnessChangedEventHandler(object sender, BrightnessChangedEventArgs e);

            private class BrightnessChangedEventArgs : EventArgs
            {
                public BrightnessChangedEventArgs(int oldBrightness, int newBrightness)
                {
                    OldBrightness = oldBrightness;
                    NewBrightness = newBrightness;
                }

                public int OldBrightness { get; private set; }
                public int NewBrightness { get; private set; }
            }
        }

        private const string PuzzleInput = @"turn on 489,959 through 759,964
